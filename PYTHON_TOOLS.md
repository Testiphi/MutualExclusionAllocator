...
默认导出仍写入 `gauntlet_data.xlsx`；默认格式化仍写回源 JSON。
...
行按 `(大地图, 小地图)` 匹配，列按车辆和星级标题匹配，不受行列移动影响。
普通/自动使用 `✓`；特殊跑法使用地图、车辆、星级、区档、跑法类型复合键。
重复行、重复车辆列、重复特殊跑法键和公式单元格会报错，避免静默丢值。
请先把公式转换为明确的实测数值。
